const app = new Vue({
  ...
  data: {
    ...
    async mounted(){
      ...
      await fetch("https://api.propublica.org/congress/v1/116/house/members.json",{headers: {"X-Api-Key": this.apiKey}})
        .then((response) => {
    ...
    methods:{
      ...
      sort2:function(s) { // added for 2nd table filter
        if (s === this.currentSort2) {
          this.currentSortDir2 = this.currentSortDir2 === 'desc' ? 'asc' : 'desc';
        }
        this.currentSort2 = s;
      },
      sort3: function (s) {
